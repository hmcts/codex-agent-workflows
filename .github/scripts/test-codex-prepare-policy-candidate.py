...
class PreparePolicyCandidateTests(unittest.TestCase):
    ...
    def test_complete_repair_patch_is_applied_to_original_source_tree(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            modified_path.write_text("first implementation\n", encoding="utf-8")
            deleted_path.unlink()
            created_path.write_text("generated by implementation\n", encoding="utf-8")
            self.git(repository, "add", "-A")
            self.git(repository, "commit", "-m", "first implementation")

            modified_path.write_text("after repair\n", encoding="utf-8")
            created_path.write_text("replaced by repair\n", encoding="utf-8")
            self.git(repository, "add", "-A")
            ...
            self.assertEqual(created_path.read_text(encoding="utf-8"), "replaced by repair\n")
            self.assertEqual(

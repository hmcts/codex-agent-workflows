...
case "${CODEX_OPERATION}" in
  jira-generate)
    ...
    PR_BODY_PATH="${pr_body_path}" PLAN_PATH="${PLAN_DIR}/plan.json" PLAN_SHA_PATH="${PLAN_DIR}/plan.sha256" \
...
Codex ran on the Azure AKS team-managed runner scale set using the Jira issue context.
